...
3. Iteratively add the best predicted variable remaining from the sorted list while R2-ajusted increase. 
...
```{r}
lm_add_variable <- 
  function(lm, variable_names){
    target <- 
      get_target_name(lm)
    predictors <- 
      get_predictors_names(lm_test)
    predictors_str <- 
    ...
    formula <- 
      ...
      as.formula()
    new_lm <- 
      lm(formula, data = lm$model)
  }
```

```{r}
lm_from_names <- 
  function(df, target, predictors){
    predictors_str <- 
      paste(predictors, collapse = " + ")
    formula_str <- 
      paste(target, predictors_str, sep = " ~ ")
