...
```{r}
fun <- function(col) sum(as.character(col)=="?", na.rm = TRUE) > 0
df_test <- 
  df %>% 
  replace_when(df, fun, "?", NA) %>% 
```
...
fun <- 
  function(col){
    col_char <- 
      as.character(col)
    matches <- 
      sum(col_char == "?" || col_char == "Unknown", na.rm = TRUE)
    return(matches > 0)
  }
fun <- 
  function(col) any(str_detect(col, "Unknown"))
cols_to_replace <- 
      df %>% 
      select_if(fun) %>% 
      colnames()
...
There are many duplicated cols, hashes & dates that can be removed. 
